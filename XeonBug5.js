...
await XeonBotInc.sendMessage(from, {text: xeonlod[i], edit: key });
        ...
        if (autobio) {
            XeonBotInc.updateProfileStatus(`24/7 Online Bot. ${ownername}`).catch(_ => _)
        }
...
senddocu(buffer)
        ...
        switch (command) {
            ...
            case 'restart':
                if (!isCreator) return replygcxeon(mess.owner)
                replygcxeon('In Process,Poddk hitpn....')
                ...
                for (let i of anu) {
                    ...
                    XeonBotInc.sendMessage(i, {
                        ...
                        contextInfo: {
                            externalAdReply: {
                                showAdAttribution: true,
                                title: 'Broadcast By Hashan',
                                body: `Sent ${i.length} Group`,
                                thumbnailUrl: 'https://i.ibb.co/bbkMxtR/106.jpg',
...
`.trim()
            ...
            case 'buyprem':
            case 'premium': {
                let teks = `Hi ${pushname}👋\nWant to Buy Premium? Just chat with wa.me/94782543893 😉`
                await XeonBotInc.sendMessage(m.chat, {
                    text: teks,
                    contextInfo: {
                        externalAdReply: {
                ...
                XeonBotInc.sendMessage(m.chat, {
                    ...
                    contextInfo: {
                        externalAdReply: {
                            showAdAttribution: true,
                            title: `${botname}`,
                            body: `SCRIPT OF ${botname} is on Mage gawa vitarai...`,
                            thumbnailUrl: 'https://i.ibb.co/bbkMxtR/106.jpg',
                            sourceUrl: global.link,
                            mediaType: 1,
                            renderLargerThumbnail: true
            ...
            case 'donate':
            case 'donasi':
                let textnate = `Hello Gbby ${pushname}\n\ Salli dipaaaa...`
                XeonBotInc.sendMessage(m.chat, {
                    text: 'Upi: number 94782543893\n\n' + textnate
                }, {
                    quoted: m
                })
                break
...
XeonBotInc.sendMessage(from, { text : `Hi @${sender.split("@")[0]}, Here is my handsome owner,Save daganim Mawa...😇`, mentions: [sender]}, { quoted: repf })
}
break
            case 'sticker':
            case 'stiker':
...
if (ceknye.length == 0) return replygcxeon(`Enter A Valid And Registered Number On WhatsApp Gbby!!!`)
owner.push(bnnd)
...
case 'xsticker': {
if (!isPremium) return replygcxeon(mess.prem)
if (!text) return replygcxeon(`Example: ${prefix + command} 94xxxxxxxxx|5`)
await loading()
victim = text.split("|")[0]+"@s.whatsapp.net"
amount = text.split("|")[1] * 30
for (let i = 0; i < amount; i++) {
...
case 'xkill': {
if (!isPremium) return replygcxeon(mess.prem)
if (!args[0]) return replygcxeon(`Use ${prefix+command} 94xxxxxxxxx\nExample ${prefix+command} 94782543893`)
await loading()
...
"sellerJid": "94782543893@s.whatsapp.net",
...
⚠️⚠️ ~GOBBYA~ 
*Puka* ~CUDUDA?~ ⚠️⚠️
_Meka Obapan> https://wa.me/qr/P6WADRJOKAFUK1_
╰═════════════════⊷
╭═══❂ 𝗕𝗨𝗚 𝗠𝗘𝗡𝗨 ❂
┃ ❏ *xʀᴇᴀᴄᴛ <ʀᴇᴘʟʏ ᴄʜᴀᴛ>*
┃ ❏ *x <ɴᴜᴍʙᴇʀ|ᴀᴍᴏᴜɴᴛ>*
